   ...
   if($name == NULL || $email == NULL || $comment == NULL)
	{
		...
		<html>
			...
			<body>
				<main>
					<h1>Error</h1>
					<p>Feild inputs are missing. Please fill out all fields.</p>
	...
	{
		addComment(trim($name), trim($email), trim($comment), trim($posted_date));
		?>
		<!DOCTYPE html>
		<html>
		...
		<p><a href="comments.php">Back to HTML Form</a></p>
			<main>
				...
				<p>Posted Date: <?php echo $posted_date; ?></p>
			</main>
		</body>
		</html>
		<?php 
...
elseif($action == 'edit')
{
	...
	{
		//Show the edit form populated with the data from the record
		//Note that we use a 'hidden' input element to include the record id in the form
	?>
	<!DOCTYPE html>
	<html>
		...
		<body>
			<main>
				...
				<textarea name="comment" id="comment" rows="5" cols="50"><?php echo $comment ?></textarea><br>
	...
	if($name == NULL || $email == NULL || $comment == NULL)
	{
		...
		<html>
			...
			<body>
				<main>
					<h1>Error</h1>
					<p>Field inputs are missing. Please fill out all fields.</p>
					<p><a href="comments.php">Back to HTML Form</a></p>
				</main>
			</body>
		<?php
		...
		updateComment($id, $name, $email, $comment, $posted_date);
...
{
	//User is trying to delete a specific comment.
...
else
{
	...
	if($comments != NULL){
		...
		foreach($comments as $row){
			...
			$commentList .= "<p>Name: $name Email: $email Posted Date: $posted_date <a href = 'comments.php?action=edit&id=$id'>edit</a> | <a href = 'comments.php?action=delete&id=$id'>delete</a></p><p>$comment</p><hr>";
		}
	...
	<html>
		...
		<body>
			<main>
				...
				<label for="comments">Comments</label><br>
				<textarea name="comment" id="comment" rows="5" cols="50"></textarea><br>
